...
# Budgets to evaluate
budget_sizes = [100, 150, 200, 250, 300]
gini_values = []
...
for budget in budget_sizes:
    ...
    E = calculate_GINI(initial_accessibility)
    ...
    gini_values.append(optimal_E)

# Plot the Gini coefficients for each budget
plt.figure()
plt.plot(budget_sizes, gini_values, marker='o', linestyle='-', color='blue')
plt.ylim(0, 1)  # Set y-axis limits between 0 and 1
plt.xlabel('Budget')
plt.ylabel('GINI Coefficient')
plt.title('Equity After Optimization for Different Budget sizes')
plt.grid(True)
plt.show()
