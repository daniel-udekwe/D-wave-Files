...
capacity_of_link_after_hurricane[0, 1] = 0
capacity_of_link_after_hurricane[0, 2] = 0
capacity_of_link_after_hurricane[1, 0] = 0
...
# Define node positions to mimic the layout in the uploaded image
positions = {
    1: (0, 4), 2: (3, 4), 
...
# Manually add all edges based on the provided network image, considering both directions
edges = [
    ...
    (22, 23),   (23, 22),   (23, 24),   (24, 23),   (24, 13),   (13, 24)
]

# Add all nodes to the graph first
G.add_nodes_from(positions.keys())

# Add edges to the graph only if the capacity is greater than 0
for i, j in edges:
    if optimal_restored_capacity[i-1, j-1] > 0:
        G.add_edge(i, j, weight=optimal_restored_capacity[i-1, j-1])

# Draw the network
plt.figure(figsize=(12, 10))

# Draw the nodes
nx.draw_networkx_nodes(G, positions, node_color='lightgreen', node_size=300)

# Draw the edges with labels for capacities
nx.draw_networkx_edges(G, positions, edgelist=G.edges, arrowstyle='-|>', arrowsize=10, connectionstyle='arc3,rad=0.1')

# Create edge labels that show both capacities for each direction
edge_labels = {}
for i, j in G.edges:
    if (j, i) in G.edges:  # Check if the reverse edge exists to label both directions
        label = f"{G[i][j]['weight']:.2f} \n {G[j][i]['weight']:.2f}"
    else:
        label = f"{G[i][j]['weight']:.2f}"
    edge_labels[(i, j)] = label

# Position edge labels above the links and ensure both capacities are visible
nx.draw_networkx_edge_labels(G, positions, edge_labels=edge_labels, font_size=12, label_pos=0.5, verticalalignment='bottom')

# Draw node labels
nx.draw_networkx_labels(G, positions, font_size=12, font_color="black")

plt.title('Restored Capacity Map')
